...
async function fetchWithRetry(id: string, maxRetries = 2): Promise<{ paste: Paste; burned: boolean }> {
  let lastErr: Error = new Error("Unknown error");
  for (let attempt = 0; attempt <= maxRetries; attempt++) {
    try {
      return await pastesApi.getById(id);
    } catch (err) {
      lastErr = err instanceof Error ? err : new Error(String(err));
      const code = (err as Error & { code?: string }).code;
      if (code === "PASTE_NOT_FOUND" || code === "PASTE_EXPIRED") {
        throw lastErr;
      }
